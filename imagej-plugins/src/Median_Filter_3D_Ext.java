/*
 * Median_Filter_3D_Ext.java - 3D Median Filter ImageJ Plugin
 * (External Process Version)
 *
 * Compile:
 *   javac -cp ij.jar Median_Filter_3D_Ext.java
 *
 * Install:
 *   Place Median_Filter_3D_Ext.class and tif_mdf/tif_mdf_g in plugins/
 */
...
public class Median_Filter_3D_Ext implements PlugInFilter {
    ...
    private static final String CPU_EXE_WIN = "tif_mdf.exe";
    private static final String CPU_EXE_UNIX = "tif_mdf";
    private static final String GPU_EXE_WIN = "tif_mdf_g.exe";
    private static final String GPU_EXE_UNIX = "tif_mdf_g";

    @Override
    public int setup(String arg, ImagePlus imp) {
        this.imp = imp;
        if (imp == null) {
            IJ.error("Median Filter 3D Ext", "No image is open");
            return DONE;
        }
        if (imp.getStackSize() < 2) {
            IJ.error("Median Filter 3D Ext",
    ...
    public void run(ImageProcessor ip) {
        ...
        IJ.log("=== 3D Median Filter (External) ===");
        ...
        if (result != null) {
            result.setTitle(imp.getTitle() + "_MDF3D");
            result.show();
        }
    }

    private boolean showDialog() {
        GenericDialog gd = new NonBlockingGenericDialog("3D Median Filter (External)");
        ...
        if (kernelSize < 3 || kernelSize > 21 || kernelSize % 2 == 0) {
            IJ.error("Median Filter 3D Ext", "Kernel size must be odd and between 3 and 21");
            return false;
        }
        return true;
    }
    ...
    private ImagePlus processWithExternalTool() {
        ...
        try {
            Path tempBase = Files.createTempDirectory("imagej_mdf3d_");
            ...
            if (!tempInputDir.mkdirs() || !tempOutputDir.mkdirs()) {
                IJ.error("Median Filter 3D Ext", "Cannot create temporary directories");
                return null;
            }
            IJ.showStatus("Saving stack as TIFF files...");
            if (!saveStackAsTiffs(imp, tempInputDir)) {
                IJ.error("Median Filter 3D Ext", "Failed to save stack");
                return null;
            }
            String exePath = findExecutable();
            IJ.log("Executable: " + exePath);
            IJ.showStatus("Running external Median filter...");
            ...
            IJ.error("Median Filter 3D Ext", "I/O error: " + e.getMessage());
            return null;
        } finally {
            if (!keepTempFiles) {
