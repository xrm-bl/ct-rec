/*
 * BM4D_Filter_3D_Ext.java - 3D BM4D Filter ImageJ Plugin
 * (External Process Version - calls tif_bm4d_g)
 *
 * Compile: javac -cp ij.jar BM4D_Filter_3D_Ext.java
 * Install: Place BM4D_Filter_3D_Ext.class and tif_bm4d_g in plugins/
 */
...
public class BM4D_Filter_3D_Ext implements PlugInFilter {
    ...
    private static final String EXE_WIN = "tif_bm4d_g.exe";
    private static final String EXE_UNIX = "tif_bm4d_g";

    @Override
    public int setup(String arg, ImagePlus imp) {
        this.imp = imp;
        if (imp == null) { IJ.error("BM4D Filter 3D Ext", "No image is open"); return DONE; }
        if (imp.getStackSize() < 2) { IJ.error("BM4D Filter 3D Ext", "Requires a stack"); return DONE; }
        int bd = imp.getBitDepth();
        if (bd != 8 && bd != 16 && bd != 32) { IJ.error("BM4D Filter 3D Ext", "Use 8/16/32-bit images"); return DONE; }
    ...
    private boolean showDialog() {
        ...
        gd.addMessage("Sigma=-1: auto-estimate from noise\n"
                + "Warning: BM4D is computationally intensive.\n"
                + "Use small search_radius for large data.\n\n"
        ...
        if (blockRadius < 1 || blockRadius > 4) { IJ.error("Block radius must be 1-4"); return false; }
        if (searchRadius < 1 || searchRadius > 10) { IJ.error("Search radius must be 1-10"); return false; }
        return true;
    }
    ...
    private ImagePlus processWithExternalTool() {
        ...
        try {
            Path tb = Files.createTempDirectory("imagej_bm4d_");
            ...
            IJ.showStatus("Running BM4D filter...");
    ...
    private int runTool(String exe, File inDir, File outDir) throws IOException {
        ...
        cmd.add(String.valueOf(blockRadius)); cmd.add(String.valueOf(searchRadius));
        if (sigma > 0) cmd.add(String.valueOf(sigma));
