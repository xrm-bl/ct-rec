/*
 * TV_Denoise_3D_Ext.java - 3D Total Variation Denoising ImageJ Plugin
 * (External Process Version - calls tif_tvd_g)
 *
 * Compile: javac -cp ij.jar TV_Denoise_3D_Ext.java
 * Install: Place TV_Denoise_3D_Ext.class and tif_tvd_g in plugins/
 */
...
public class TV_Denoise_3D_Ext implements PlugInFilter {
    ...
    private static final String EXE_WIN = "tif_tvd_g.exe";
    private static final String EXE_UNIX = "tif_tvd_g";

    @Override
    public int setup(String arg, ImagePlus imp) {
        this.imp = imp;
        if (imp == null) { IJ.error("TV Denoise 3D Ext", "No image is open"); return DONE; }
        if (imp.getStackSize() < 2) { IJ.error("TV Denoise 3D Ext", "Requires a stack"); return DONE; }
        int bd = imp.getBitDepth();
        if (bd != 8 && bd != 16 && bd != 32) { IJ.error("TV Denoise 3D Ext", "Use 8/16/32-bit images"); return DONE; }
    ...
    public void run(ImageProcessor ip) {
        ...
        IJ.log("=== 3D TV Denoising (External GPU) ===");
        IJ.log("Image: " + imp.getTitle() + " (" + imp.getWidth() + "x" + imp.getHeight() + "x" + imp.getStackSize() + ")");
        IJ.log("Lambda: " + lambda + ", Iterations: " + iterations);
        ImagePlus result = processWithExternalTool();
        IJ.log("Total time: " + ((System.currentTimeMillis() - t0) / 1000.0) + " seconds");
        if (result != null) { result.setTitle(imp.getTitle() + "_TVD3D"); result.show(); }
    }

    private boolean showDialog() {
        GenericDialog gd = new NonBlockingGenericDialog("3D TV Denoising (External GPU)");
        gd.addNumericField("Lambda (-1=auto, larger=less denoising):", lambda, 2);
    ...
    private ImagePlus processWithExternalTool() {
        ...
        try {
            Path tb = Files.createTempDirectory("imagej_tvd3d_");
            ...
            IJ.showStatus("Running TV denoising...");
    ...
    private int runTool(String exe, File inDir, File outDir) throws IOException {
        ...
        if (lambda > 0) cmd.add(String.valueOf(lambda));
        if (lambda > 0 || iterations != 100) { if (lambda <= 0) cmd.add("-1"); cmd.add(String.valueOf(iterations)); }
