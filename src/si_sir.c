/*
 * si_sir.c - 3D binning (Bx x By x Bz block average) of a slice stack
 *
 * 2026-09-24: rewritten from the original si_sir.c / si_sir_pipe.c.
 * AI: Claude Fable 5 (Anthropic)
 *
 * The block arithmetic is identical to the original si_sir.c. The
 * OpenMP machinery of si_sir_pipe.c (thread-local sum buffers +
 * reduction, double buffering) was removed: this job is I/O-bound
 * and the plain loop is faster than the parallel bookkeeping.
 *
 * I/O goes through libtiff - build with rif_tiff.c (read) and
 * sif_tiff.c (write) plus TIFFLIB, not rif.c/rif_fast.c/rsi.c/
 * sif.c/sif_fast.c. That switch alone made slice reads ~8x faster
 * than rif_fast.c (see 20260717_LIBTIFF-MIGRATION.md).
 */
...
int main(int argc, char **argv)
{
    ...
    v = (Ny - 1) % By + 1; xv = (double)Bx * (double)v; hv = (double)h * (double)v;
    {
        ...
        (void)sprintf(form, "%s/%%0%dd.tif", argv[argc - 1], l);
    }
    for (z = 0; z < Nz; z++) {
        ...
        for (y = 0; y < Ny; y++) {
            v = y / By;
            for (x = 0; x < Nx; x++) sum[v][x / Bx] += (double)cell[y][x];
        }
        if ((z + 1) % Bz == 0 || z + 1 == Nz) {
