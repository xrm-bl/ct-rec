...
/* Calculate overlap based on max(half_kernel, 3*spatial_sigma) + safety margin */
...
/* Save only the valid (non-overlap) region of a chunk */
...
static void get_pixel_value(void *data, ImageInfo *info, int x, int y, int z, double *value) {
    size_t offset;
    void *pixel_data;
    offset = z * info->bytes_per_slice + y * info->width * info->bytes_per_pixel + x * info->bytes_per_pixel;
...
/* Apply bilateral filter to the valid region of a chunk */
static void apply_bilateral_filter_chunk(ChunkData *chunk, ImageInfo *info, FilterParams *params) {
    void *output_data;
    int x, y, z, kx, ky, kz;
    int half_kernel;
...
static double get_float_dynamic_range(const char *dir_path, char files[][MAX_PATH_LENGTH],
    ...
    for (i = 0; i < sample_count; i++) {
        ...
        start_y = info->height / 4;
        end_y = 3 * info->height / 4;
        y_step = (end_y - start_y) / 10;
        if (y_step < 1) y_step = 1;
        for (y = start_y; y < end_y; y += y_step) {
            if (TIFFReadScanline(tif, buffer, y, 0) >= 0) {
                start_x = info->width / 4;
                end_x = 3 * info->width / 4;
...
int main(int argc, char *argv[]) {
    ...
    if (file_count == 0) {
        fprintf(stderr, "Error: No TIFF files found in input directory\n");
        free(files); return 1;
    }
