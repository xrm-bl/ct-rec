...
/*----------------------------------------------------------------------*/
/* TIFF (.tif) full-frame reader: malloc global data[Nx*Ny], set Nx,Ny.  */
int read_tiff(char *fname)
{
    TIFF     *tif;
    uint32_t w, hgt;
    int      y;
    ...
    Nx = (unsigned short)w;
    Ny = (unsigned short)hgt;

    data = (unsigned short *) malloc((size_t)Nx*Ny*sizeof(unsigned short));
    if(data == NULL){ TIFFClose(tif); fprintf(stderr,"no memory for %s\n",fname); return(-12); }

    for(y=0; y<Ny; y++){
        if(TIFFReadScanline(tif, data+(size_t)y*Nx, y, 0) < 0){
            TIFFClose(tif); free(data);
...
/*----------------------------------------------------------------------*/
/* dispatch: img -> read_hipic, tif -> read_tiff (".img" name -> ".tif"). */
int read_frame(char *fname, Header *h)
{
    char tn[2048];
    int  L;

    if(!useTiff) return read_hipic(fname, h);

    snprintf(tn, sizeof(tn), "%s", fname);
    L = (int)strlen(tn);
    if(L >= 4 && strcmp(tn+L-4, ".img") == 0) strcpy(tn+L-4, ".tif");
    return read_tiff(tn);
}
...
//IIO[j] and IIO[j+1] are opened
		if(iFlag==0) snprintf(fname, sizeof(fname), "%s/q%03d.img", dirin, II0[j]);
		if(iFlag==1) snprintf(fname, sizeof(fname), "%s/q%04d.img", dirin, II0[j]);
		if(iFlag==2) snprintf(fname, sizeof(fname), "%s/q%05d.img", dirin, II0[j]);
		if ((i = read_frame(fname, &h)) != 0){
...
//		}
		...
		if(iFlag==0) snprintf(fname, sizeof(fname), "%s/q%03d.img", dirin, II0[j+1]);
		if(iFlag==1) snprintf(fname, sizeof(fname), "%s/q%04d.img", dirin, II0[j+1]);
		if(iFlag==2) snprintf(fname, sizeof(fname), "%s/q%05d.img", dirin, II0[j+1]);
		if ((i = read_frame(fname, &h)) != 0){
...
// 1 layer(ln)
		...
		for ( k = II0[j] + 1; k < II0[j+1]; ++k){
			...
			if(iFlag==2) snprintf(fname, sizeof(fname), "%s/q%05d.img", dirin, k);
			if ((i = read_frame(fname, &h)) != 0){
				printf("something wrong -- return value is %d(II01)", i);
				return(-1);
			}
...
// read shot log file
    ...
    /* detect input format from the dark frame present in the input dir */
    {
        char dpath[1024];
        FILE *ftest;
        snprintf(dpath, sizeof(dpath), "%s/dark.img", argv[1]);
        if((ftest = fopen(dpath, "rb")) != NULL){ fclose(ftest); useTiff = 0; }
        else{
            snprintf(dpath, sizeof(dpath), "%s/dark.tif", argv[1]);
            if((ftest = fopen(dpath, "rb")) != NULL){ fclose(ftest); useTiff = 1; }
            else{ fprintf(stderr, "dark.img / dark.tif not found in %s\n", argv[1]); return(1); }
        }
    }

// read dark image
	snprintf(darkfile, sizeof(darkfile),"%s/dark.img",argv[1]);
	if ((i = read_frame(darkfile, &h)) != 0){
