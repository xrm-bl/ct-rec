...
// 1 layer(ln)
		...
		for ( k = II0[j] + 1; k < II0[j+1]; ++k){
			...
			for(i=0;i<Nx*Ny;++i){
				*(I+i)=*(data+i)-*(dark+i);
				Is=Is+*(I+i);
				*(I0+i)=*(a+i) * shottime[k] + *(b+i);
				I0s=I0s+*(I0+i);
			}
			free(data);
			if(cFlag==0) I0cor=Is/I0s;   //
			//if(cFlag==1) I0cor=1.0;      // for no correction
...
//		fprintf(stderr, "parameter was wrong!!!\n");
		fprintf(stderr, "usage : %s HiPic/ prj/ (I0cor)\n", argv[0]);
