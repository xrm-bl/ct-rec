// program ofact_rec  (offset-CT reconstruction for the automatic CT system)
//
// Same reconstruction as ofct_rec, but the normalization range is fixed
...
//
// Required files are q???.tif (or q???.img), dark.tif/dark.img, output.log.
...
static void Store8TiffFile(char *wname, int wX, int wY, int wBPS, unsigned char *data8, char *wdesc)
{
	...
	TIFFSetField(image, TIFFTAG_ARTIST, "ofact_rec");
...
// decided before execution, so all six arguments are required.
	if (argc==7){
		...
		fprintf(stderr, "usage : %s layer offset pixel_size offsetangle LACmin LACmax\n", argv[0]);
...
// "ofct_rec (32bit) -> tif_f2i" chain.  The frame is trimmed to an even
// size exactly as tif_f2i does.
	cN = NN; if (cN%2==1) cN=cN-1;
		...
		Error("output memory allocation error.");
{
	...
	for(vv=0;vv<cN;vv++){
		for(hh=0;hh<cN;hh++){
			double	v=(double)(float)*(rec_temp+(size_t)NN*vv+hh)-d_min;
...
// pixel, axis, Nproj, angle, data min, data max, norm min, norm max
	sprintf(comm,"%f\t%f\t%d\t%f\t%f\t%f\t%lf\t%lf",
		f_size, f_center, NNST, theta0,
		(float)data_min, (float)data_max, d_min, d_max);

	(void)snprintf(fout,  sizeof(fout),  "ro%05d.tif", (int)s_layer);
	Store8TiffFile (fout,  cN, cN,  8, data8,  comm);
	(void)snprintf(fout2, sizeof(fout2), "rh%05d.tif", (int)s_layer);
	Store16TiffFile(fout2, cN, cN, 16, data16, comm);
	free(data8); free(data16);
	printf("%s\t%s\t%f\t%f\t%d\t%f\t%f\t%f\t%lf\t%lf\t%f\t%f\n",fout,fout2,
		f_size, f_center, NNST, theta0,
		(float)data_min, (float)data_max, d_min, d_max, (float)t1, (float)t2);

	free(pp); free(rec_temp);
